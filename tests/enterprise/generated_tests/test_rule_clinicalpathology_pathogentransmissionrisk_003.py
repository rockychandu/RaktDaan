"""
Automated Test Suite for ClinicalPathology - PathogenTransmissionRisk (3) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_clinicalpathology_pathogentransmissionrisk_003 import ClinicalPathologyPathogenTransmissionRiskClinicalProcessor3


def test_clinicalpathology_pathogentransmissionrisk_3_evaluation():
    processor = ClinicalPathologyPathogenTransmissionRiskClinicalProcessor3()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_clinicalpathology_pathogentransmissionrisk_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_CLINICALPATHOLOGY_PATHOGENTRANSMISSIONRISK_003"
