"""
Automated Test Suite for Nanomedicine - PhysiologicalVitalsThreshold (1) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_nanomedicine_physiologicalvitalsthreshold_001 import NanomedicinePhysiologicalVitalsThresholdClinicalProcessor1


def test_nanomedicine_physiologicalvitalsthreshold_1_evaluation():
    processor = NanomedicinePhysiologicalVitalsThresholdClinicalProcessor1()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_nanomedicine_physiologicalvitalsthreshold_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_NANOMEDICINE_PHYSIOLOGICALVITALSTHRESHOLD_001"
