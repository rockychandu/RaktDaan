"""
Automated Test Suite for TransfusionMedicine - PathogenTransmissionRisk (2) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_transfusionmedicine_pathogentransmissionrisk_002 import TransfusionMedicinePathogenTransmissionRiskClinicalProcessor2


def test_transfusionmedicine_pathogentransmissionrisk_2_evaluation():
    processor = TransfusionMedicinePathogenTransmissionRiskClinicalProcessor2()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_transfusionmedicine_pathogentransmissionrisk_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_TRANSFUSIONMEDICINE_PATHOGENTRANSMISSIONRISK_002"
