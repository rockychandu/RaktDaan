"""
Automated Test Suite for ClinicalPathology - PathogenTransmissionRisk (1) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_clinicalpathology_pathogentransmissionrisk_001 import ClinicalPathologyPathogenTransmissionRiskClinicalProcessor1


def test_clinicalpathology_pathogentransmissionrisk_1_evaluation():
    processor = ClinicalPathologyPathogenTransmissionRiskClinicalProcessor1()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_clinicalpathology_pathogentransmissionrisk_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_CLINICALPATHOLOGY_PATHOGENTRANSMISSIONRISK_001"
