"""
Automated Test Suite for TransplantImmunology - PostOperativeRecovery (1) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_transplantimmunology_postoperativerecovery_001 import TransplantImmunologyPostOperativeRecoveryClinicalProcessor1


def test_transplantimmunology_postoperativerecovery_1_evaluation():
    processor = TransplantImmunologyPostOperativeRecoveryClinicalProcessor1()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_transplantimmunology_postoperativerecovery_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_TRANSPLANTIMMUNOLOGY_POSTOPERATIVERECOVERY_001"
