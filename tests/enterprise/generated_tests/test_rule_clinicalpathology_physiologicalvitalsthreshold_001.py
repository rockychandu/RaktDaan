"""
Automated Test Suite for ClinicalPathology - PhysiologicalVitalsThreshold (1) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_clinicalpathology_physiologicalvitalsthreshold_001 import ClinicalPathologyPhysiologicalVitalsThresholdClinicalProcessor1


def test_clinicalpathology_physiologicalvitalsthreshold_1_evaluation():
    processor = ClinicalPathologyPhysiologicalVitalsThresholdClinicalProcessor1()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_clinicalpathology_physiologicalvitalsthreshold_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_CLINICALPATHOLOGY_PHYSIOLOGICALVITALSTHRESHOLD_001"
