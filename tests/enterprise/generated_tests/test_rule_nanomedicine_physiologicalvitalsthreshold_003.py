"""
Automated Test Suite for Nanomedicine - PhysiologicalVitalsThreshold (3) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_nanomedicine_physiologicalvitalsthreshold_003 import NanomedicinePhysiologicalVitalsThresholdClinicalProcessor3


def test_nanomedicine_physiologicalvitalsthreshold_3_evaluation():
    processor = NanomedicinePhysiologicalVitalsThresholdClinicalProcessor3()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_nanomedicine_physiologicalvitalsthreshold_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_NANOMEDICINE_PHYSIOLOGICALVITALSTHRESHOLD_003"
