"""
Automated Test Suite for TransplantImmunology - ImmuneComplexRejection (2) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_transplantimmunology_immunecomplexrejection_002 import TransplantImmunologyImmuneComplexRejectionClinicalProcessor2


def test_transplantimmunology_immunecomplexrejection_2_evaluation():
    processor = TransplantImmunologyImmuneComplexRejectionClinicalProcessor2()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_transplantimmunology_immunecomplexrejection_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_TRANSPLANTIMMUNOLOGY_IMMUNECOMPLEXREJECTION_002"
