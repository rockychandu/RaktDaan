"""
Automated Test Suite for CellularTherapy - DiagnosticLaboratoryMarker (1) Clinical Rule Processor.
"""

from app.enterprise.generated_rules.rule_cellulartherapy_diagnosticlaboratorymarker_001 import CellularTherapyDiagnosticLaboratoryMarkerClinicalProcessor1


def test_cellulartherapy_diagnosticlaboratorymarker_1_evaluation():
    processor = CellularTherapyDiagnosticLaboratoryMarkerClinicalProcessor1()
    
    # Test primary fail
    res_fail = processor.evaluate_primary_condition({"has_cellulartherapy_diagnosticlaboratorymarker_primary": True})
    assert res_fail["passed"] is False
    assert res_fail["rule_code"] == "RULE_ENT_CELLULARTHERAPY_DIAGNOSTICLABORATORYMARKER_001"
