"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - GeneticRiskFactor (1).
...
class InfectiousDiseaseGeneticRiskFactorClinicalProcessor1:
    """
    Detailed decision processor evaluating InfectiousDisease (GeneticRiskFactor) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_GENETICRISKFACTOR_001"
    rule_name = "InfectiousDisease GeneticRiskFactor Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (GeneticRiskFactor)."""
        has_primary = donor_payload.get("has_infectiousdisease_geneticriskfactor_primary", False)
        severity_score = donor_payload.get("infectiousdisease_geneticriskfactor_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_geneticriskfactor_lab_marker", 10.0)
