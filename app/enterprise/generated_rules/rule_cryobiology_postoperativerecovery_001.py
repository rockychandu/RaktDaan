"""
Enterprise Clinical Screening & Medical Rule Engine: Cryobiology - PostOperativeRecovery (1).
...
class CryobiologyPostOperativeRecoveryClinicalProcessor1:
    """
    Detailed decision processor evaluating Cryobiology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CRYOBIOLOGY_POSTOPERATIVERECOVERY_001"
    rule_name = "Cryobiology PostOperativeRecovery Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Cryobiology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_cryobiology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("cryobiology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("cryobiology_postoperativerecovery_lab_marker", 10.0)
