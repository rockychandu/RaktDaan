"""
Enterprise Clinical Screening & Medical Rule Engine: Immunology - PediatricSafetyProtocol (3).
...
class ImmunologyPediatricSafetyProtocolClinicalProcessor3:
    """
    Detailed decision processor evaluating Immunology (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_IMMUNOLOGY_PEDIATRICSAFETYPROTOCOL_003"
    rule_name = "Immunology PediatricSafetyProtocol Screening Rule #3"
    category = "IMMUNOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Immunology (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_immunology_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("immunology_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("immunology_pediatricsafetyprotocol_lab_marker", 10.0)
