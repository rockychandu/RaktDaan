"""
Enterprise Clinical Screening & Medical Rule Engine: Neurology - ImmuneComplexRejection (3).
...
class NeurologyImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Neurology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_NEUROLOGY_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Neurology ImmuneComplexRejection Screening Rule #3"
    category = "NEUROLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Neurology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_neurology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("neurology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("neurology_immunecomplexrejection_lab_marker", 10.0)
