"""
Enterprise Clinical Screening & Medical Rule Engine: Pediatrics - ImmuneComplexRejection (2).
...
class PediatricsImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating Pediatrics (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PEDIATRICS_IMMUNECOMPLEXREJECTION_002"
    rule_name = "Pediatrics ImmuneComplexRejection Screening Rule #2"
    category = "PEDIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Pediatrics (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_pediatrics_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("pediatrics_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("pediatrics_immunecomplexrejection_lab_marker", 10.0)
