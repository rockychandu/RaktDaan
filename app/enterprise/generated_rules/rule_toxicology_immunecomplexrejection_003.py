"""
Enterprise Clinical Screening & Medical Rule Engine: Toxicology - ImmuneComplexRejection (3).
...
class ToxicologyImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Toxicology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TOXICOLOGY_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Toxicology ImmuneComplexRejection Screening Rule #3"
    category = "TOXICOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Toxicology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_toxicology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("toxicology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("toxicology_immunecomplexrejection_lab_marker", 10.0)
