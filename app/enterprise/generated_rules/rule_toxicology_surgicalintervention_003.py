"""
Enterprise Clinical Screening & Medical Rule Engine: Toxicology - SurgicalIntervention (3).
...
class ToxicologySurgicalInterventionClinicalProcessor3:
    """
    Detailed decision processor evaluating Toxicology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TOXICOLOGY_SURGICALINTERVENTION_003"
    rule_name = "Toxicology SurgicalIntervention Screening Rule #3"
    category = "TOXICOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Toxicology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_toxicology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("toxicology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("toxicology_surgicalintervention_lab_marker", 10.0)
