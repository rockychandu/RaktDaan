"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - PostOperativeRecovery (2).
...
class ApheresisPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Apheresis (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_POSTOPERATIVERECOVERY_002"
    rule_name = "Apheresis PostOperativeRecovery Screening Rule #2"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_apheresis_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("apheresis_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_postoperativerecovery_lab_marker", 10.0)
