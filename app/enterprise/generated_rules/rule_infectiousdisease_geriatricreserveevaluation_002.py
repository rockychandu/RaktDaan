"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - GeriatricReserveEvaluation (2).
...
class InfectiousDiseaseGeriatricReserveEvaluationClinicalProcessor2:
    """
    Detailed decision processor evaluating InfectiousDisease (GeriatricReserveEvaluation) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_GERIATRICRESERVEEVALUATION_002"
    rule_name = "InfectiousDisease GeriatricReserveEvaluation Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (GeriatricReserveEvaluation)."""
        has_primary = donor_payload.get("has_infectiousdisease_geriatricreserveevaluation_primary", False)
        severity_score = donor_payload.get("infectiousdisease_geriatricreserveevaluation_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_geriatricreserveevaluation_lab_marker", 10.0)
