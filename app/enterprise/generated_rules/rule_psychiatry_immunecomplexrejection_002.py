"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - ImmuneComplexRejection (2).
...
class PsychiatryImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating Psychiatry (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_IMMUNECOMPLEXREJECTION_002"
    rule_name = "Psychiatry ImmuneComplexRejection Screening Rule #2"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_psychiatry_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("psychiatry_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("psychiatry_immunecomplexrejection_lab_marker", 10.0)
