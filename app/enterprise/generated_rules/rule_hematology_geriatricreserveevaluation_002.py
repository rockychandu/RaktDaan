"""
Enterprise Clinical Screening & Medical Rule Engine: Hematology - GeriatricReserveEvaluation (2).
...
class HematologyGeriatricReserveEvaluationClinicalProcessor2:
    """
    Detailed decision processor evaluating Hematology (GeriatricReserveEvaluation) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HEMATOLOGY_GERIATRICRESERVEEVALUATION_002"
    rule_name = "Hematology GeriatricReserveEvaluation Screening Rule #2"
    category = "HEMATOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Hematology (GeriatricReserveEvaluation)."""
        has_primary = donor_payload.get("has_hematology_geriatricreserveevaluation_primary", False)
        severity_score = donor_payload.get("hematology_geriatricreserveevaluation_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("hematology_geriatricreserveevaluation_lab_marker", 10.0)
