"""
Enterprise Clinical Screening & Medical Rule Engine: Rheumatology - PostOperativeRecovery (2).
...
class RheumatologyPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Rheumatology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_RHEUMATOLOGY_POSTOPERATIVERECOVERY_002"
    rule_name = "Rheumatology PostOperativeRecovery Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Rheumatology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_rheumatology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("rheumatology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("rheumatology_postoperativerecovery_lab_marker", 10.0)
