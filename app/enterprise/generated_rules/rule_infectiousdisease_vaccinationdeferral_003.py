"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - VaccinationDeferral (3).
...
class InfectiousDiseaseVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating InfectiousDisease (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_VACCINATIONDEFERRAL_003"
    rule_name = "InfectiousDisease VaccinationDeferral Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_infectiousdisease_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("infectiousdisease_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_vaccinationdeferral_lab_marker", 10.0)
