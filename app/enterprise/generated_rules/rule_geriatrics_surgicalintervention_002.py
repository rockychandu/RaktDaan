"""
Enterprise Clinical Screening & Medical Rule Engine: Geriatrics - SurgicalIntervention (2).
...
class GeriatricsSurgicalInterventionClinicalProcessor2:
    """
    Detailed decision processor evaluating Geriatrics (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GERIATRICS_SURGICALINTERVENTION_002"
    rule_name = "Geriatrics SurgicalIntervention Screening Rule #2"
    category = "GERIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Geriatrics (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_geriatrics_surgicalintervention_primary", False)
        severity_score = donor_payload.get("geriatrics_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("geriatrics_surgicalintervention_lab_marker", 10.0)
