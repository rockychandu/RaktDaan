"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - SurgicalIntervention (2).
...
class ApheresisSurgicalInterventionClinicalProcessor2:
    """
    Detailed decision processor evaluating Apheresis (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_SURGICALINTERVENTION_002"
    rule_name = "Apheresis SurgicalIntervention Screening Rule #2"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_apheresis_surgicalintervention_primary", False)
        severity_score = donor_payload.get("apheresis_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_surgicalintervention_lab_marker", 10.0)
