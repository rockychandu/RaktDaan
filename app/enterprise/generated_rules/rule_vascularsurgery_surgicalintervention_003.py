"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - SurgicalIntervention (3).
...
class VascularSurgerySurgicalInterventionClinicalProcessor3:
    """
    Detailed decision processor evaluating VascularSurgery (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_SURGICALINTERVENTION_003"
    rule_name = "VascularSurgery SurgicalIntervention Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_vascularsurgery_surgicalintervention_primary", False)
        severity_score = donor_payload.get("vascularsurgery_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("vascularsurgery_surgicalintervention_lab_marker", 10.0)
