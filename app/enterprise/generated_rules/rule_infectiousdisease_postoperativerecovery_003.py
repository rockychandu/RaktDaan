"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - PostOperativeRecovery (3).
...
class InfectiousDiseasePostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating InfectiousDisease (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_POSTOPERATIVERECOVERY_003"
    rule_name = "InfectiousDisease PostOperativeRecovery Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_infectiousdisease_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("infectiousdisease_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_postoperativerecovery_lab_marker", 10.0)
