"""
Enterprise Clinical Screening & Medical Rule Engine: Radiology - ImmuneComplexRejection (3).
...
class RadiologyImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Radiology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_RADIOLOGY_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Radiology ImmuneComplexRejection Screening Rule #3"
    category = "RADIOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Radiology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_radiology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("radiology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("radiology_immunecomplexrejection_lab_marker", 10.0)
