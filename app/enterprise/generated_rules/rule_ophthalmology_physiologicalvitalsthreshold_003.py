"""
Enterprise Clinical Screening & Medical Rule Engine: Ophthalmology - PhysiologicalVitalsThreshold (3).
...
class OphthalmologyPhysiologicalVitalsThresholdClinicalProcessor3:
    """
    Detailed decision processor evaluating Ophthalmology (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_OPHTHALMOLOGY_PHYSIOLOGICALVITALSTHRESHOLD_003"
    rule_name = "Ophthalmology PhysiologicalVitalsThreshold Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Ophthalmology (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_ophthalmology_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("ophthalmology_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Ophthalmology."""
        lab_marker = donor_payload.get("ophthalmology_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_ophthalmology", False)
