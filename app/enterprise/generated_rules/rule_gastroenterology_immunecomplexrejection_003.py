"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - ImmuneComplexRejection (3).
...
class GastroenterologyImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Gastroenterology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Gastroenterology ImmuneComplexRejection Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_gastroenterology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("gastroenterology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_immunecomplexrejection_lab_marker", 10.0)
