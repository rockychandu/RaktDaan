"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - PediatricSafetyProtocol (3).
...
class VascularSurgeryPediatricSafetyProtocolClinicalProcessor3:
    """
    Detailed decision processor evaluating VascularSurgery (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_PEDIATRICSAFETYPROTOCOL_003"
    rule_name = "VascularSurgery PediatricSafetyProtocol Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_vascularsurgery_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("vascularsurgery_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("vascularsurgery_pediatricsafetyprotocol_lab_marker", 10.0)
