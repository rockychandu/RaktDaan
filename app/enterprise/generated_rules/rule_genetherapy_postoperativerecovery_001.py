"""
Enterprise Clinical Screening & Medical Rule Engine: GeneTherapy - PostOperativeRecovery (1).
...
class GeneTherapyPostOperativeRecoveryClinicalProcessor1:
    """
    Detailed decision processor evaluating GeneTherapy (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GENETHERAPY_POSTOPERATIVERECOVERY_001"
    rule_name = "GeneTherapy PostOperativeRecovery Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for GeneTherapy (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_genetherapy_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("genetherapy_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("genetherapy_postoperativerecovery_lab_marker", 10.0)
