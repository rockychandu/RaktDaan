"""
Enterprise Clinical Screening & Medical Rule Engine: Cryobiology - PediatricSafetyProtocol (3).
...
class CryobiologyPediatricSafetyProtocolClinicalProcessor3:
    """
    Detailed decision processor evaluating Cryobiology (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CRYOBIOLOGY_PEDIATRICSAFETYPROTOCOL_003"
    rule_name = "Cryobiology PediatricSafetyProtocol Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Cryobiology (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_cryobiology_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("cryobiology_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("cryobiology_pediatricsafetyprotocol_lab_marker", 10.0)
