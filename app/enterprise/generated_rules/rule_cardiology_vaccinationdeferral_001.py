"""
Enterprise Clinical Screening & Medical Rule Engine: Cardiology - VaccinationDeferral (1).
...
class CardiologyVaccinationDeferralClinicalProcessor1:
    """
    Detailed decision processor evaluating Cardiology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CARDIOLOGY_VACCINATIONDEFERRAL_001"
    rule_name = "Cardiology VaccinationDeferral Screening Rule #1"
    category = "CARDIOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Cardiology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_cardiology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("cardiology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("cardiology_vaccinationdeferral_lab_marker", 10.0)
