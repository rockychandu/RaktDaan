"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - SurgicalIntervention (1).
...
class PsychiatrySurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating Psychiatry (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_SURGICALINTERVENTION_001"
    rule_name = "Psychiatry SurgicalIntervention Screening Rule #1"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_psychiatry_surgicalintervention_primary", False)
        severity_score = donor_payload.get("psychiatry_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("psychiatry_surgicalintervention_lab_marker", 10.0)
