"""
Enterprise Clinical Screening & Medical Rule Engine: TransfusionMedicine - PathogenTransmissionRisk (3).
...
class TransfusionMedicinePathogenTransmissionRiskClinicalProcessor3:
    """
    Detailed decision processor evaluating TransfusionMedicine (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSFUSIONMEDICINE_PATHOGENTRANSMISSIONRISK_003"
    rule_name = "TransfusionMedicine PathogenTransmissionRisk Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransfusionMedicine (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_transfusionmedicine_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("transfusionmedicine_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transfusionmedicine_pathogentransmissionrisk_lab_marker", 10.0)
