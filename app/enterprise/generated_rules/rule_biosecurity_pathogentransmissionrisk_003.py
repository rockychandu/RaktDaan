"""
Enterprise Clinical Screening & Medical Rule Engine: Biosecurity - PathogenTransmissionRisk (3).
...
class BiosecurityPathogenTransmissionRiskClinicalProcessor3:
    """
    Detailed decision processor evaluating Biosecurity (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_BIOSECURITY_PATHOGENTRANSMISSIONRISK_003"
    rule_name = "Biosecurity PathogenTransmissionRisk Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Biosecurity (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_biosecurity_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("biosecurity_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("biosecurity_pathogentransmissionrisk_lab_marker", 10.0)
