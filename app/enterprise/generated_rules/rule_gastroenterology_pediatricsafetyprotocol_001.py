"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - PediatricSafetyProtocol (1).
...
class GastroenterologyPediatricSafetyProtocolClinicalProcessor1:
    """
    Detailed decision processor evaluating Gastroenterology (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_PEDIATRICSAFETYPROTOCOL_001"
    rule_name = "Gastroenterology PediatricSafetyProtocol Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_gastroenterology_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("gastroenterology_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_pediatricsafetyprotocol_lab_marker", 10.0)
