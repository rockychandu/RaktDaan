"""
Enterprise Clinical Screening & Medical Rule Engine: Ophthalmology - PostOperativeRecovery (3).
...
class OphthalmologyPostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating Ophthalmology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_OPHTHALMOLOGY_POSTOPERATIVERECOVERY_003"
    rule_name = "Ophthalmology PostOperativeRecovery Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Ophthalmology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_ophthalmology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("ophthalmology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("ophthalmology_postoperativerecovery_lab_marker", 10.0)
