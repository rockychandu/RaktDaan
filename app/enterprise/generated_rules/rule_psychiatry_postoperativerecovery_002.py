"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - PostOperativeRecovery (2).
...
class PsychiatryPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Psychiatry (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_POSTOPERATIVERECOVERY_002"
    rule_name = "Psychiatry PostOperativeRecovery Screening Rule #2"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_psychiatry_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("psychiatry_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("psychiatry_postoperativerecovery_lab_marker", 10.0)
