"""
Enterprise Clinical Screening & Medical Rule Engine: TransplantImmunology - PostOperativeRecovery (3).
...
class TransplantImmunologyPostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating TransplantImmunology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSPLANTIMMUNOLOGY_POSTOPERATIVERECOVERY_003"
    rule_name = "TransplantImmunology PostOperativeRecovery Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransplantImmunology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_transplantimmunology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("transplantimmunology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transplantimmunology_postoperativerecovery_lab_marker", 10.0)
