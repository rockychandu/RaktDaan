"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - DiagnosticLaboratoryMarker (1).
...
class SurgeryDiagnosticLaboratoryMarkerClinicalProcessor1:
    """
    Detailed decision processor evaluating Surgery (DiagnosticLaboratoryMarker) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_DIAGNOSTICLABORATORYMARKER_001"
    rule_name = "Surgery DiagnosticLaboratoryMarker Screening Rule #1"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (DiagnosticLaboratoryMarker)."""
        has_primary = donor_payload.get("has_surgery_diagnosticlaboratorymarker_primary", False)
        severity_score = donor_payload.get("surgery_diagnosticlaboratorymarker_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("surgery_diagnosticlaboratorymarker_lab_marker", 10.0)
