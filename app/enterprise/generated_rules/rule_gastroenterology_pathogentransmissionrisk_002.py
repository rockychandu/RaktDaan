"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - PathogenTransmissionRisk (2).
...
class GastroenterologyPathogenTransmissionRiskClinicalProcessor2:
    """
    Detailed decision processor evaluating Gastroenterology (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_PATHOGENTRANSMISSIONRISK_002"
    rule_name = "Gastroenterology PathogenTransmissionRisk Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_gastroenterology_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("gastroenterology_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_pathogentransmissionrisk_lab_marker", 10.0)
