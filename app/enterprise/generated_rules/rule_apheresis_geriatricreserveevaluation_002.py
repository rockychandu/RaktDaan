"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - GeriatricReserveEvaluation (2).
...
class ApheresisGeriatricReserveEvaluationClinicalProcessor2:
    """
    Detailed decision processor evaluating Apheresis (GeriatricReserveEvaluation) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_GERIATRICRESERVEEVALUATION_002"
    rule_name = "Apheresis GeriatricReserveEvaluation Screening Rule #2"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (GeriatricReserveEvaluation)."""
        has_primary = donor_payload.get("has_apheresis_geriatricreserveevaluation_primary", False)
        severity_score = donor_payload.get("apheresis_geriatricreserveevaluation_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_geriatricreserveevaluation_lab_marker", 10.0)
