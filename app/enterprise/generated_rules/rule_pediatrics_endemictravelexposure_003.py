"""
Enterprise Clinical Screening & Medical Rule Engine: Pediatrics - EndemicTravelExposure (3).
...
class PediatricsEndemicTravelExposureClinicalProcessor3:
    """
    Detailed decision processor evaluating Pediatrics (EndemicTravelExposure) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PEDIATRICS_ENDEMICTRAVELEXPOSURE_003"
    rule_name = "Pediatrics EndemicTravelExposure Screening Rule #3"
    category = "PEDIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Pediatrics (EndemicTravelExposure)."""
        has_primary = donor_payload.get("has_pediatrics_endemictravelexposure_primary", False)
        severity_score = donor_payload.get("pediatrics_endemictravelexposure_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("pediatrics_endemictravelexposure_lab_marker", 10.0)
