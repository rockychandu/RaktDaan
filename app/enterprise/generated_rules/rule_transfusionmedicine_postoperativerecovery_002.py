"""
Enterprise Clinical Screening & Medical Rule Engine: TransfusionMedicine - PostOperativeRecovery (2).
...
class TransfusionMedicinePostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating TransfusionMedicine (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSFUSIONMEDICINE_POSTOPERATIVERECOVERY_002"
    rule_name = "TransfusionMedicine PostOperativeRecovery Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransfusionMedicine (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_transfusionmedicine_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("transfusionmedicine_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transfusionmedicine_postoperativerecovery_lab_marker", 10.0)
