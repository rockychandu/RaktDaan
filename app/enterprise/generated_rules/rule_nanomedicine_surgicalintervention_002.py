"""
Enterprise Clinical Screening & Medical Rule Engine: Nanomedicine - SurgicalIntervention (2).
...
class NanomedicineSurgicalInterventionClinicalProcessor2:
    """
    Detailed decision processor evaluating Nanomedicine (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_NANOMEDICINE_SURGICALINTERVENTION_002"
    rule_name = "Nanomedicine SurgicalIntervention Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Nanomedicine (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_nanomedicine_surgicalintervention_primary", False)
        severity_score = donor_payload.get("nanomedicine_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("nanomedicine_surgicalintervention_lab_marker", 10.0)
