"""
Enterprise Clinical Screening & Medical Rule Engine: GeneTherapy - PhysiologicalVitalsThreshold (3).
...
class GeneTherapyPhysiologicalVitalsThresholdClinicalProcessor3:
    """
    Detailed decision processor evaluating GeneTherapy (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GENETHERAPY_PHYSIOLOGICALVITALSTHRESHOLD_003"
    rule_name = "GeneTherapy PhysiologicalVitalsThreshold Screening Rule #3"
    category = "GENETHERAPY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for GeneTherapy (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_genetherapy_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("genetherapy_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for GeneTherapy."""
        lab_marker = donor_payload.get("genetherapy_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_genetherapy", False)
