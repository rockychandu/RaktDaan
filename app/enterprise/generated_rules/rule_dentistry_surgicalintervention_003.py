"""
Enterprise Clinical Screening & Medical Rule Engine: Dentistry - SurgicalIntervention (3).
...
class DentistrySurgicalInterventionClinicalProcessor3:
    """
    Detailed decision processor evaluating Dentistry (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_DENTISTRY_SURGICALINTERVENTION_003"
    rule_name = "Dentistry SurgicalIntervention Screening Rule #3"
    category = "DENTISTRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Dentistry (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_dentistry_surgicalintervention_primary", False)
        severity_score = donor_payload.get("dentistry_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("dentistry_surgicalintervention_lab_marker", 10.0)
