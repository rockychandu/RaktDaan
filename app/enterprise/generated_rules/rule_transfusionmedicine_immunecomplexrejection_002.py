"""
Enterprise Clinical Screening & Medical Rule Engine: TransfusionMedicine - ImmuneComplexRejection (2).
...
class TransfusionMedicineImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating TransfusionMedicine (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSFUSIONMEDICINE_IMMUNECOMPLEXREJECTION_002"
    rule_name = "TransfusionMedicine ImmuneComplexRejection Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransfusionMedicine (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_transfusionmedicine_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("transfusionmedicine_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transfusionmedicine_immunecomplexrejection_lab_marker", 10.0)
