"""
Enterprise Clinical Screening & Medical Rule Engine: Dentistry - ImmuneComplexRejection (3).
...
class DentistryImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Dentistry (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_DENTISTRY_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Dentistry ImmuneComplexRejection Screening Rule #3"
    category = "DENTISTRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Dentistry (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_dentistry_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("dentistry_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("dentistry_immunecomplexrejection_lab_marker", 10.0)
