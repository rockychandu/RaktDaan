"""
Enterprise Clinical Screening & Medical Rule Engine: Hematology - VaccinationDeferral (2).
...
class HematologyVaccinationDeferralClinicalProcessor2:
    """
    Detailed decision processor evaluating Hematology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HEMATOLOGY_VACCINATIONDEFERRAL_002"
    rule_name = "Hematology VaccinationDeferral Screening Rule #2"
    category = "HEMATOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Hematology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_hematology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("hematology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("hematology_vaccinationdeferral_lab_marker", 10.0)
