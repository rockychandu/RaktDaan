"""
Enterprise Clinical Screening & Medical Rule Engine: ClinicalPathology - PostOperativeRecovery (1).
...
class ClinicalPathologyPostOperativeRecoveryClinicalProcessor1:
    """
    Detailed decision processor evaluating ClinicalPathology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CLINICALPATHOLOGY_POSTOPERATIVERECOVERY_001"
    rule_name = "ClinicalPathology PostOperativeRecovery Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for ClinicalPathology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_clinicalpathology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("clinicalpathology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("clinicalpathology_postoperativerecovery_lab_marker", 10.0)
