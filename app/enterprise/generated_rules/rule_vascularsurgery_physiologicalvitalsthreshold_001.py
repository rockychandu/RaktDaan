"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - PhysiologicalVitalsThreshold (1).
...
class VascularSurgeryPhysiologicalVitalsThresholdClinicalProcessor1:
    """
    Detailed decision processor evaluating VascularSurgery (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_PHYSIOLOGICALVITALSTHRESHOLD_001"
    rule_name = "VascularSurgery PhysiologicalVitalsThreshold Screening Rule #1"
    category = "VASCULARSURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_vascularsurgery_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("vascularsurgery_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for VascularSurgery."""
        lab_marker = donor_payload.get("vascularsurgery_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_vascularsurgery", False)
