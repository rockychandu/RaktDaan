"""
Enterprise Clinical Screening & Medical Rule Engine: Nephrology - ImmuneComplexRejection (2).
...
class NephrologyImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating Nephrology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_NEPHROLOGY_IMMUNECOMPLEXREJECTION_002"
    rule_name = "Nephrology ImmuneComplexRejection Screening Rule #2"
    category = "NEPHROLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Nephrology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_nephrology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("nephrology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("nephrology_immunecomplexrejection_lab_marker", 10.0)
