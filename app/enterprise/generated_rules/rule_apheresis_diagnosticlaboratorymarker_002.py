"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - DiagnosticLaboratoryMarker (2).
...
class ApheresisDiagnosticLaboratoryMarkerClinicalProcessor2:
    """
    Detailed decision processor evaluating Apheresis (DiagnosticLaboratoryMarker) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_DIAGNOSTICLABORATORYMARKER_002"
    rule_name = "Apheresis DiagnosticLaboratoryMarker Screening Rule #2"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (DiagnosticLaboratoryMarker)."""
        has_primary = donor_payload.get("has_apheresis_diagnosticlaboratorymarker_primary", False)
        severity_score = donor_payload.get("apheresis_diagnosticlaboratorymarker_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_diagnosticlaboratorymarker_lab_marker", 10.0)
