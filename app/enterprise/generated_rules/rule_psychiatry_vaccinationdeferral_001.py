"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - VaccinationDeferral (1).
...
class PsychiatryVaccinationDeferralClinicalProcessor1:
    """
    Detailed decision processor evaluating Psychiatry (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_VACCINATIONDEFERRAL_001"
    rule_name = "Psychiatry VaccinationDeferral Screening Rule #1"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_psychiatry_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("psychiatry_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("psychiatry_vaccinationdeferral_lab_marker", 10.0)
