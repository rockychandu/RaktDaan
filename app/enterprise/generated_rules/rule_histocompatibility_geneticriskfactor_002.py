"""
Enterprise Clinical Screening & Medical Rule Engine: Histocompatibility - GeneticRiskFactor (2).
...
class HistocompatibilityGeneticRiskFactorClinicalProcessor2:
    """
    Detailed decision processor evaluating Histocompatibility (GeneticRiskFactor) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HISTOCOMPATIBILITY_GENETICRISKFACTOR_002"
    rule_name = "Histocompatibility GeneticRiskFactor Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Histocompatibility (GeneticRiskFactor)."""
        has_primary = donor_payload.get("has_histocompatibility_geneticriskfactor_primary", False)
        severity_score = donor_payload.get("histocompatibility_geneticriskfactor_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("histocompatibility_geneticriskfactor_lab_marker", 10.0)
