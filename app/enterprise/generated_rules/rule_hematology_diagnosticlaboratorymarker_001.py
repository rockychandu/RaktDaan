"""
Enterprise Clinical Screening & Medical Rule Engine: Hematology - DiagnosticLaboratoryMarker (1).
...
class HematologyDiagnosticLaboratoryMarkerClinicalProcessor1:
    """
    Detailed decision processor evaluating Hematology (DiagnosticLaboratoryMarker) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HEMATOLOGY_DIAGNOSTICLABORATORYMARKER_001"
    rule_name = "Hematology DiagnosticLaboratoryMarker Screening Rule #1"
    category = "HEMATOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Hematology (DiagnosticLaboratoryMarker)."""
        has_primary = donor_payload.get("has_hematology_diagnosticlaboratorymarker_primary", False)
        severity_score = donor_payload.get("hematology_diagnosticlaboratorymarker_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("hematology_diagnosticlaboratorymarker_lab_marker", 10.0)
