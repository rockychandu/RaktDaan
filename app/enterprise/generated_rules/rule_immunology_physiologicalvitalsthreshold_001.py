"""
Enterprise Clinical Screening & Medical Rule Engine: Immunology - PhysiologicalVitalsThreshold (1).
...
class ImmunologyPhysiologicalVitalsThresholdClinicalProcessor1:
    """
    Detailed decision processor evaluating Immunology (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_IMMUNOLOGY_PHYSIOLOGICALVITALSTHRESHOLD_001"
    rule_name = "Immunology PhysiologicalVitalsThreshold Screening Rule #1"
    category = "IMMUNOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Immunology (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_immunology_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("immunology_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Immunology."""
        lab_marker = donor_payload.get("immunology_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_immunology", False)
