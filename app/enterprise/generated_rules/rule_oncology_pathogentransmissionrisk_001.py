"""
Enterprise Clinical Screening & Medical Rule Engine: Oncology - PathogenTransmissionRisk (1).
...
class OncologyPathogenTransmissionRiskClinicalProcessor1:
    """
    Detailed decision processor evaluating Oncology (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_ONCOLOGY_PATHOGENTRANSMISSIONRISK_001"
    rule_name = "Oncology PathogenTransmissionRisk Screening Rule #1"
    category = "ONCOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Oncology (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_oncology_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("oncology_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("oncology_pathogentransmissionrisk_lab_marker", 10.0)
