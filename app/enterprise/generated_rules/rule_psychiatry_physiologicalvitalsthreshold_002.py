"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - PhysiologicalVitalsThreshold (2).
...
class PsychiatryPhysiologicalVitalsThresholdClinicalProcessor2:
    """
    Detailed decision processor evaluating Psychiatry (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_PHYSIOLOGICALVITALSTHRESHOLD_002"
    rule_name = "Psychiatry PhysiologicalVitalsThreshold Screening Rule #2"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_psychiatry_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("psychiatry_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Psychiatry."""
        lab_marker = donor_payload.get("psychiatry_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_psychiatry", False)
