"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - PediatricSafetyProtocol (2).
...
class SurgeryPediatricSafetyProtocolClinicalProcessor2:
    """
    Detailed decision processor evaluating Surgery (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_PEDIATRICSAFETYPROTOCOL_002"
    rule_name = "Surgery PediatricSafetyProtocol Screening Rule #2"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_surgery_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("surgery_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("surgery_pediatricsafetyprotocol_lab_marker", 10.0)
