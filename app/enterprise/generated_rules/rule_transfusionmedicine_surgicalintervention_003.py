"""
Enterprise Clinical Screening & Medical Rule Engine: TransfusionMedicine - SurgicalIntervention (3).
...
class TransfusionMedicineSurgicalInterventionClinicalProcessor3:
    """
    Detailed decision processor evaluating TransfusionMedicine (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSFUSIONMEDICINE_SURGICALINTERVENTION_003"
    rule_name = "TransfusionMedicine SurgicalIntervention Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransfusionMedicine (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_transfusionmedicine_surgicalintervention_primary", False)
        severity_score = donor_payload.get("transfusionmedicine_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transfusionmedicine_surgicalintervention_lab_marker", 10.0)
