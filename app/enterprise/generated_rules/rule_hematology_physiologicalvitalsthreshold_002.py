"""
Enterprise Clinical Screening & Medical Rule Engine: Hematology - PhysiologicalVitalsThreshold (2).
...
class HematologyPhysiologicalVitalsThresholdClinicalProcessor2:
    """
    Detailed decision processor evaluating Hematology (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HEMATOLOGY_PHYSIOLOGICALVITALSTHRESHOLD_002"
    rule_name = "Hematology PhysiologicalVitalsThreshold Screening Rule #2"
    category = "HEMATOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Hematology (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_hematology_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("hematology_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Hematology."""
        lab_marker = donor_payload.get("hematology_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_hematology", False)
