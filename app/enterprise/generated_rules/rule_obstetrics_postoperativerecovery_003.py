"""
Enterprise Clinical Screening & Medical Rule Engine: Obstetrics - PostOperativeRecovery (3).
...
class ObstetricsPostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating Obstetrics (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_OBSTETRICS_POSTOPERATIVERECOVERY_003"
    rule_name = "Obstetrics PostOperativeRecovery Screening Rule #3"
    category = "OBSTETRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Obstetrics (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_obstetrics_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("obstetrics_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("obstetrics_postoperativerecovery_lab_marker", 10.0)
