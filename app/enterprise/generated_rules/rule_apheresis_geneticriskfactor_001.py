"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - GeneticRiskFactor (1).
...
class ApheresisGeneticRiskFactorClinicalProcessor1:
    """
    Detailed decision processor evaluating Apheresis (GeneticRiskFactor) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_GENETICRISKFACTOR_001"
    rule_name = "Apheresis GeneticRiskFactor Screening Rule #1"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (GeneticRiskFactor)."""
        has_primary = donor_payload.get("has_apheresis_geneticriskfactor_primary", False)
        severity_score = donor_payload.get("apheresis_geneticriskfactor_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_geneticriskfactor_lab_marker", 10.0)
