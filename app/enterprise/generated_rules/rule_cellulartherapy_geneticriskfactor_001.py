"""
Enterprise Clinical Screening & Medical Rule Engine: CellularTherapy - GeneticRiskFactor (1).
...
class CellularTherapyGeneticRiskFactorClinicalProcessor1:
    """
    Detailed decision processor evaluating CellularTherapy (GeneticRiskFactor) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CELLULARTHERAPY_GENETICRISKFACTOR_001"
    rule_name = "CellularTherapy GeneticRiskFactor Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for CellularTherapy (GeneticRiskFactor)."""
        has_primary = donor_payload.get("has_cellulartherapy_geneticriskfactor_primary", False)
        severity_score = donor_payload.get("cellulartherapy_geneticriskfactor_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("cellulartherapy_geneticriskfactor_lab_marker", 10.0)
