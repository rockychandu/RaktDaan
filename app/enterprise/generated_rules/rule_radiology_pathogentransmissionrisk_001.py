"""
Enterprise Clinical Screening & Medical Rule Engine: Radiology - PathogenTransmissionRisk (1).
...
class RadiologyPathogenTransmissionRiskClinicalProcessor1:
    """
    Detailed decision processor evaluating Radiology (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_RADIOLOGY_PATHOGENTRANSMISSIONRISK_001"
    rule_name = "Radiology PathogenTransmissionRisk Screening Rule #1"
    category = "RADIOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Radiology (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_radiology_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("radiology_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("radiology_pathogentransmissionrisk_lab_marker", 10.0)
