"""
Enterprise Clinical Screening & Medical Rule Engine: Urology - ImmuneComplexRejection (1).
...
class UrologyImmuneComplexRejectionClinicalProcessor1:
    """
    Detailed decision processor evaluating Urology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_UROLOGY_IMMUNECOMPLEXREJECTION_001"
    rule_name = "Urology ImmuneComplexRejection Screening Rule #1"
    category = "UROLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Urology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_urology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("urology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("urology_immunecomplexrejection_lab_marker", 10.0)
