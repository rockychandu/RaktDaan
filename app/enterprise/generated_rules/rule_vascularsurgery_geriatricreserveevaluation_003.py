"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - GeriatricReserveEvaluation (3).
...
class VascularSurgeryGeriatricReserveEvaluationClinicalProcessor3:
    """
    Detailed decision processor evaluating VascularSurgery (GeriatricReserveEvaluation) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_GERIATRICRESERVEEVALUATION_003"
    rule_name = "VascularSurgery GeriatricReserveEvaluation Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (GeriatricReserveEvaluation)."""
        has_primary = donor_payload.get("has_vascularsurgery_geriatricreserveevaluation_primary", False)
        severity_score = donor_payload.get("vascularsurgery_geriatricreserveevaluation_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("vascularsurgery_geriatricreserveevaluation_lab_marker", 10.0)
