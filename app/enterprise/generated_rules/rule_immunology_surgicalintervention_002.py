"""
Enterprise Clinical Screening & Medical Rule Engine: Immunology - SurgicalIntervention (2).
...
class ImmunologySurgicalInterventionClinicalProcessor2:
    """
    Detailed decision processor evaluating Immunology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_IMMUNOLOGY_SURGICALINTERVENTION_002"
    rule_name = "Immunology SurgicalIntervention Screening Rule #2"
    category = "IMMUNOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Immunology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_immunology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("immunology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("immunology_surgicalintervention_lab_marker", 10.0)
