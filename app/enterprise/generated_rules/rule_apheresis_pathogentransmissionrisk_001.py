"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - PathogenTransmissionRisk (1).
...
class ApheresisPathogenTransmissionRiskClinicalProcessor1:
    """
    Detailed decision processor evaluating Apheresis (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_PATHOGENTRANSMISSIONRISK_001"
    rule_name = "Apheresis PathogenTransmissionRisk Screening Rule #1"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_apheresis_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("apheresis_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_pathogentransmissionrisk_lab_marker", 10.0)
