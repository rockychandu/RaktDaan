"""
Enterprise Clinical Screening & Medical Rule Engine: CellularTherapy - PhysiologicalVitalsThreshold (2).
...
class CellularTherapyPhysiologicalVitalsThresholdClinicalProcessor2:
    """
    Detailed decision processor evaluating CellularTherapy (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CELLULARTHERAPY_PHYSIOLOGICALVITALSTHRESHOLD_002"
    rule_name = "CellularTherapy PhysiologicalVitalsThreshold Screening Rule #2"
    category = "CELLULARTHERAPY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for CellularTherapy (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_cellulartherapy_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("cellulartherapy_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for CellularTherapy."""
        lab_marker = donor_payload.get("cellulartherapy_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_cellulartherapy", False)
