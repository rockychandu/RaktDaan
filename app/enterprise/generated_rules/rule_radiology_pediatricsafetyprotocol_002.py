"""
Enterprise Clinical Screening & Medical Rule Engine: Radiology - PediatricSafetyProtocol (2).
...
class RadiologyPediatricSafetyProtocolClinicalProcessor2:
    """
    Detailed decision processor evaluating Radiology (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_RADIOLOGY_PEDIATRICSAFETYPROTOCOL_002"
    rule_name = "Radiology PediatricSafetyProtocol Screening Rule #2"
    category = "RADIOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Radiology (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_radiology_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("radiology_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("radiology_pediatricsafetyprotocol_lab_marker", 10.0)
