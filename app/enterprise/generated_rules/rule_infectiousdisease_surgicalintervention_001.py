"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - SurgicalIntervention (1).
...
class InfectiousDiseaseSurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating InfectiousDisease (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_SURGICALINTERVENTION_001"
    rule_name = "InfectiousDisease SurgicalIntervention Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_infectiousdisease_surgicalintervention_primary", False)
        severity_score = donor_payload.get("infectiousdisease_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_surgicalintervention_lab_marker", 10.0)
