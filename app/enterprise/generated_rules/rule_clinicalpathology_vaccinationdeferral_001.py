"""
Enterprise Clinical Screening & Medical Rule Engine: ClinicalPathology - VaccinationDeferral (1).
...
class ClinicalPathologyVaccinationDeferralClinicalProcessor1:
    """
    Detailed decision processor evaluating ClinicalPathology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CLINICALPATHOLOGY_VACCINATIONDEFERRAL_001"
    rule_name = "ClinicalPathology VaccinationDeferral Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for ClinicalPathology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_clinicalpathology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("clinicalpathology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("clinicalpathology_vaccinationdeferral_lab_marker", 10.0)
