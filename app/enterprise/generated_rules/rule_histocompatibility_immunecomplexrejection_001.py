"""
Enterprise Clinical Screening & Medical Rule Engine: Histocompatibility - ImmuneComplexRejection (1).
...
class HistocompatibilityImmuneComplexRejectionClinicalProcessor1:
    """
    Detailed decision processor evaluating Histocompatibility (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HISTOCOMPATIBILITY_IMMUNECOMPLEXREJECTION_001"
    rule_name = "Histocompatibility ImmuneComplexRejection Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Histocompatibility (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_histocompatibility_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("histocompatibility_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("histocompatibility_immunecomplexrejection_lab_marker", 10.0)
