"""
Enterprise Clinical Screening & Medical Rule Engine: Oncology - ImmuneComplexRejection (2).
...
class OncologyImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating Oncology (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_ONCOLOGY_IMMUNECOMPLEXREJECTION_002"
    rule_name = "Oncology ImmuneComplexRejection Screening Rule #2"
    category = "ONCOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Oncology (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_oncology_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("oncology_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("oncology_immunecomplexrejection_lab_marker", 10.0)
