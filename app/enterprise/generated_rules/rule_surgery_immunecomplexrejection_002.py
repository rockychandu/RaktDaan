"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - ImmuneComplexRejection (2).
...
class SurgeryImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating Surgery (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_IMMUNECOMPLEXREJECTION_002"
    rule_name = "Surgery ImmuneComplexRejection Screening Rule #2"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_surgery_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("surgery_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("surgery_immunecomplexrejection_lab_marker", 10.0)
