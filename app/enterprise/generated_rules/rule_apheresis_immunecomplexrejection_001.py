"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - ImmuneComplexRejection (1).
...
class ApheresisImmuneComplexRejectionClinicalProcessor1:
    """
    Detailed decision processor evaluating Apheresis (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_IMMUNECOMPLEXREJECTION_001"
    rule_name = "Apheresis ImmuneComplexRejection Screening Rule #1"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_apheresis_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("apheresis_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_immunecomplexrejection_lab_marker", 10.0)
