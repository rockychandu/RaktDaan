"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - PathogenTransmissionRisk (3).
...
class SurgeryPathogenTransmissionRiskClinicalProcessor3:
    """
    Detailed decision processor evaluating Surgery (PathogenTransmissionRisk) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_PATHOGENTRANSMISSIONRISK_003"
    rule_name = "Surgery PathogenTransmissionRisk Screening Rule #3"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (PathogenTransmissionRisk)."""
        has_primary = donor_payload.get("has_surgery_pathogentransmissionrisk_primary", False)
        severity_score = donor_payload.get("surgery_pathogentransmissionrisk_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("surgery_pathogentransmissionrisk_lab_marker", 10.0)
