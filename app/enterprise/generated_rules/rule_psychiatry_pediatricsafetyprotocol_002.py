"""
Enterprise Clinical Screening & Medical Rule Engine: Psychiatry - PediatricSafetyProtocol (2).
...
class PsychiatryPediatricSafetyProtocolClinicalProcessor2:
    """
    Detailed decision processor evaluating Psychiatry (PediatricSafetyProtocol) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PSYCHIATRY_PEDIATRICSAFETYPROTOCOL_002"
    rule_name = "Psychiatry PediatricSafetyProtocol Screening Rule #2"
    category = "PSYCHIATRY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Psychiatry (PediatricSafetyProtocol)."""
        has_primary = donor_payload.get("has_psychiatry_pediatricsafetyprotocol_primary", False)
        severity_score = donor_payload.get("psychiatry_pediatricsafetyprotocol_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("psychiatry_pediatricsafetyprotocol_lab_marker", 10.0)
