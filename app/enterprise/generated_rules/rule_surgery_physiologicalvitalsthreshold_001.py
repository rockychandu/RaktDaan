"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - PhysiologicalVitalsThreshold (1).
...
class SurgeryPhysiologicalVitalsThresholdClinicalProcessor1:
    """
    Detailed decision processor evaluating Surgery (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_PHYSIOLOGICALVITALSTHRESHOLD_001"
    rule_name = "Surgery PhysiologicalVitalsThreshold Screening Rule #1"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_surgery_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("surgery_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Surgery."""
        lab_marker = donor_payload.get("surgery_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_surgery", False)
