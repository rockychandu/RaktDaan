"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - PhysiologicalVitalsThreshold (3).
...
class ApheresisPhysiologicalVitalsThresholdClinicalProcessor3:
    """
    Detailed decision processor evaluating Apheresis (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_PHYSIOLOGICALVITALSTHRESHOLD_003"
    rule_name = "Apheresis PhysiologicalVitalsThreshold Screening Rule #3"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_apheresis_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("apheresis_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Apheresis."""
        lab_marker = donor_payload.get("apheresis_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_apheresis", False)
