"""
Enterprise Clinical Screening & Medical Rule Engine: Radiology - SurgicalIntervention (1).
...
class RadiologySurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating Radiology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_RADIOLOGY_SURGICALINTERVENTION_001"
    rule_name = "Radiology SurgicalIntervention Screening Rule #1"
    category = "RADIOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Radiology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_radiology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("radiology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("radiology_surgicalintervention_lab_marker", 10.0)
