"""
Enterprise Clinical Screening & Medical Rule Engine: Obstetrics - ImmuneComplexRejection (3).
...
class ObstetricsImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Obstetrics (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_OBSTETRICS_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Obstetrics ImmuneComplexRejection Screening Rule #3"
    category = "OBSTETRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Obstetrics (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_obstetrics_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("obstetrics_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("obstetrics_immunecomplexrejection_lab_marker", 10.0)
