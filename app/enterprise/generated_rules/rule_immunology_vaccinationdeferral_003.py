"""
Enterprise Clinical Screening & Medical Rule Engine: Immunology - VaccinationDeferral (3).
...
class ImmunologyVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating Immunology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_IMMUNOLOGY_VACCINATIONDEFERRAL_003"
    rule_name = "Immunology VaccinationDeferral Screening Rule #3"
    category = "IMMUNOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Immunology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_immunology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("immunology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("immunology_vaccinationdeferral_lab_marker", 10.0)
