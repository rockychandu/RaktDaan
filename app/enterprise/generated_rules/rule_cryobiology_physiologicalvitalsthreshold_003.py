"""
Enterprise Clinical Screening & Medical Rule Engine: Cryobiology - PhysiologicalVitalsThreshold (3).
...
class CryobiologyPhysiologicalVitalsThresholdClinicalProcessor3:
    """
    Detailed decision processor evaluating Cryobiology (PhysiologicalVitalsThreshold) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CRYOBIOLOGY_PHYSIOLOGICALVITALSTHRESHOLD_003"
    rule_name = "Cryobiology PhysiologicalVitalsThreshold Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Cryobiology (PhysiologicalVitalsThreshold)."""
        has_primary = donor_payload.get("has_cryobiology_physiologicalvitalsthreshold_primary", False)
        severity_score = donor_payload.get("cryobiology_physiologicalvitalsthreshold_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates secondary lab markers, medication interactions, and vital ranges for Cryobiology."""
        lab_marker = donor_payload.get("cryobiology_physiologicalvitalsthreshold_lab_marker", 10.0)
        is_medicated = donor_payload.get("is_medicated_cryobiology", False)
