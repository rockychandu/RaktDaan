"""
Enterprise Clinical Screening & Medical Rule Engine: ClinicalPathology - SurgicalIntervention (1).
...
class ClinicalPathologySurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating ClinicalPathology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_CLINICALPATHOLOGY_SURGICALINTERVENTION_001"
    rule_name = "ClinicalPathology SurgicalIntervention Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for ClinicalPathology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_clinicalpathology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("clinicalpathology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("clinicalpathology_surgicalintervention_lab_marker", 10.0)
