"""
Enterprise Clinical Screening & Medical Rule Engine: Hematology - PostOperativeRecovery (2).
...
class HematologyPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Hematology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HEMATOLOGY_POSTOPERATIVERECOVERY_002"
    rule_name = "Hematology PostOperativeRecovery Screening Rule #2"
    category = "HEMATOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Hematology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_hematology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("hematology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("hematology_postoperativerecovery_lab_marker", 10.0)
