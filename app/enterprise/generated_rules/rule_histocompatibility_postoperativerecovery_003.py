"""
Enterprise Clinical Screening & Medical Rule Engine: Histocompatibility - PostOperativeRecovery (3).
...
class HistocompatibilityPostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating Histocompatibility (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_HISTOCOMPATIBILITY_POSTOPERATIVERECOVERY_003"
    rule_name = "Histocompatibility PostOperativeRecovery Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Histocompatibility (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_histocompatibility_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("histocompatibility_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("histocompatibility_postoperativerecovery_lab_marker", 10.0)
