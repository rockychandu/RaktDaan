"""
Enterprise Clinical Screening & Medical Rule Engine: TransfusionMedicine - MedicationTherapy (1).
...
class TransfusionMedicineMedicationTherapyClinicalProcessor1:
    """
    Detailed decision processor evaluating TransfusionMedicine (MedicationTherapy) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSFUSIONMEDICINE_MEDICATIONTHERAPY_001"
    rule_name = "TransfusionMedicine MedicationTherapy Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransfusionMedicine (MedicationTherapy)."""
        has_primary = donor_payload.get("has_transfusionmedicine_medicationtherapy_primary", False)
        severity_score = donor_payload.get("transfusionmedicine_medicationtherapy_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transfusionmedicine_medicationtherapy_lab_marker", 10.0)
