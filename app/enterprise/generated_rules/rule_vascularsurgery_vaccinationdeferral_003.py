"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - VaccinationDeferral (3).
...
class VascularSurgeryVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating VascularSurgery (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_VACCINATIONDEFERRAL_003"
    rule_name = "VascularSurgery VaccinationDeferral Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_vascularsurgery_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("vascularsurgery_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("vascularsurgery_vaccinationdeferral_lab_marker", 10.0)
