"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - VaccinationDeferral (3).
...
class GastroenterologyVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating Gastroenterology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_VACCINATIONDEFERRAL_003"
    rule_name = "Gastroenterology VaccinationDeferral Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_gastroenterology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("gastroenterology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_vaccinationdeferral_lab_marker", 10.0)
