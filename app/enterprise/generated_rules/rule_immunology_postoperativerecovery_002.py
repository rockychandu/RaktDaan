"""
Enterprise Clinical Screening & Medical Rule Engine: Immunology - PostOperativeRecovery (2).
...
class ImmunologyPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Immunology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_IMMUNOLOGY_POSTOPERATIVERECOVERY_002"
    rule_name = "Immunology PostOperativeRecovery Screening Rule #2"
    category = "IMMUNOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Immunology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_immunology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("immunology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("immunology_postoperativerecovery_lab_marker", 10.0)
