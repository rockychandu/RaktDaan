"""
Enterprise Clinical Screening & Medical Rule Engine: Toxicology - VaccinationDeferral (3).
...
class ToxicologyVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating Toxicology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TOXICOLOGY_VACCINATIONDEFERRAL_003"
    rule_name = "Toxicology VaccinationDeferral Screening Rule #3"
    category = "TOXICOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Toxicology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_toxicology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("toxicology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("toxicology_vaccinationdeferral_lab_marker", 10.0)
