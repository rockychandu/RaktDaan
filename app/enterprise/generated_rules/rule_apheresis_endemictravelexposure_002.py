"""
Enterprise Clinical Screening & Medical Rule Engine: Apheresis - EndemicTravelExposure (2).
...
class ApheresisEndemicTravelExposureClinicalProcessor2:
    """
    Detailed decision processor evaluating Apheresis (EndemicTravelExposure) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_APHERESIS_ENDEMICTRAVELEXPOSURE_002"
    rule_name = "Apheresis EndemicTravelExposure Screening Rule #2"
    category = "APHERESIS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Apheresis (EndemicTravelExposure)."""
        has_primary = donor_payload.get("has_apheresis_endemictravelexposure_primary", False)
        severity_score = donor_payload.get("apheresis_endemictravelexposure_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("apheresis_endemictravelexposure_lab_marker", 10.0)
