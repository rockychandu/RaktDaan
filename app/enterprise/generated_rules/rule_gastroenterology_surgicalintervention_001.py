"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - SurgicalIntervention (1).
...
class GastroenterologySurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating Gastroenterology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_SURGICALINTERVENTION_001"
    rule_name = "Gastroenterology SurgicalIntervention Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_gastroenterology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("gastroenterology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_surgicalintervention_lab_marker", 10.0)
