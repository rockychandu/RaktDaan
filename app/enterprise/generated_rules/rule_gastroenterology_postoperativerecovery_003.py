"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - PostOperativeRecovery (3).
...
class GastroenterologyPostOperativeRecoveryClinicalProcessor3:
    """
    Detailed decision processor evaluating Gastroenterology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_POSTOPERATIVERECOVERY_003"
    rule_name = "Gastroenterology PostOperativeRecovery Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_gastroenterology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("gastroenterology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_postoperativerecovery_lab_marker", 10.0)
