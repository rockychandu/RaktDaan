"""
Enterprise Clinical Screening & Medical Rule Engine: Geriatrics - ImmuneComplexRejection (3).
...
class GeriatricsImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Geriatrics (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GERIATRICS_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Geriatrics ImmuneComplexRejection Screening Rule #3"
    category = "GERIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Geriatrics (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_geriatrics_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("geriatrics_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("geriatrics_immunecomplexrejection_lab_marker", 10.0)
