"""
Enterprise Clinical Screening & Medical Rule Engine: Biosecurity - ImmuneComplexRejection (1).
...
class BiosecurityImmuneComplexRejectionClinicalProcessor1:
    """
    Detailed decision processor evaluating Biosecurity (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_BIOSECURITY_IMMUNECOMPLEXREJECTION_001"
    rule_name = "Biosecurity ImmuneComplexRejection Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Biosecurity (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_biosecurity_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("biosecurity_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("biosecurity_immunecomplexrejection_lab_marker", 10.0)
