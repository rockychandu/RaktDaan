"""
Enterprise Clinical Screening & Medical Rule Engine: VascularSurgery - PostOperativeRecovery (1).
...
class VascularSurgeryPostOperativeRecoveryClinicalProcessor1:
    """
    Detailed decision processor evaluating VascularSurgery (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_VASCULARSURGERY_POSTOPERATIVERECOVERY_001"
    rule_name = "VascularSurgery PostOperativeRecovery Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for VascularSurgery (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_vascularsurgery_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("vascularsurgery_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("vascularsurgery_postoperativerecovery_lab_marker", 10.0)
