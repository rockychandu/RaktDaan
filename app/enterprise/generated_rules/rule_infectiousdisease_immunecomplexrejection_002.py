"""
Enterprise Clinical Screening & Medical Rule Engine: InfectiousDisease - ImmuneComplexRejection (2).
...
class InfectiousDiseaseImmuneComplexRejectionClinicalProcessor2:
    """
    Detailed decision processor evaluating InfectiousDisease (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_INFECTIOUSDISEASE_IMMUNECOMPLEXREJECTION_002"
    rule_name = "InfectiousDisease ImmuneComplexRejection Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for InfectiousDisease (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_infectiousdisease_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("infectiousdisease_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("infectiousdisease_immunecomplexrejection_lab_marker", 10.0)
