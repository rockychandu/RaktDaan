"""
Enterprise Clinical Screening & Medical Rule Engine: Surgery - GeriatricReserveEvaluation (2).
...
class SurgeryGeriatricReserveEvaluationClinicalProcessor2:
    """
    Detailed decision processor evaluating Surgery (GeriatricReserveEvaluation) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_SURGERY_GERIATRICRESERVEEVALUATION_002"
    rule_name = "Surgery GeriatricReserveEvaluation Screening Rule #2"
    category = "SURGERY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Surgery (GeriatricReserveEvaluation)."""
        has_primary = donor_payload.get("has_surgery_geriatricreserveevaluation_primary", False)
        severity_score = donor_payload.get("surgery_geriatricreserveevaluation_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("surgery_geriatricreserveevaluation_lab_marker", 10.0)
