"""
Enterprise Clinical Screening & Medical Rule Engine: Pediatrics - SurgicalIntervention (3).
...
class PediatricsSurgicalInterventionClinicalProcessor3:
    """
    Detailed decision processor evaluating Pediatrics (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_PEDIATRICS_SURGICALINTERVENTION_003"
    rule_name = "Pediatrics SurgicalIntervention Screening Rule #3"
    category = "PEDIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Pediatrics (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_pediatrics_surgicalintervention_primary", False)
        severity_score = donor_payload.get("pediatrics_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("pediatrics_surgicalintervention_lab_marker", 10.0)
