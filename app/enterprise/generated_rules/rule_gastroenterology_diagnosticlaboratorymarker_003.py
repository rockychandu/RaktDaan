"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - DiagnosticLaboratoryMarker (3).
...
class GastroenterologyDiagnosticLaboratoryMarkerClinicalProcessor3:
    """
    Detailed decision processor evaluating Gastroenterology (DiagnosticLaboratoryMarker) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_DIAGNOSTICLABORATORYMARKER_003"
    rule_name = "Gastroenterology DiagnosticLaboratoryMarker Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (DiagnosticLaboratoryMarker)."""
        has_primary = donor_payload.get("has_gastroenterology_diagnosticlaboratorymarker_primary", False)
        severity_score = donor_payload.get("gastroenterology_diagnosticlaboratorymarker_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_diagnosticlaboratorymarker_lab_marker", 10.0)
