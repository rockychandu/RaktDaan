"""
Enterprise Clinical Screening & Medical Rule Engine: Oncology - PostOperativeRecovery (2).
...
class OncologyPostOperativeRecoveryClinicalProcessor2:
    """
    Detailed decision processor evaluating Oncology (PostOperativeRecovery) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_ONCOLOGY_POSTOPERATIVERECOVERY_002"
    rule_name = "Oncology PostOperativeRecovery Screening Rule #2"
    category = "ONCOLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Oncology (PostOperativeRecovery)."""
        has_primary = donor_payload.get("has_oncology_postoperativerecovery_primary", False)
        severity_score = donor_payload.get("oncology_postoperativerecovery_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("oncology_postoperativerecovery_lab_marker", 10.0)
