"""
Enterprise Clinical Screening & Medical Rule Engine: Geriatrics - VaccinationDeferral (3).
...
class GeriatricsVaccinationDeferralClinicalProcessor3:
    """
    Detailed decision processor evaluating Geriatrics (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GERIATRICS_VACCINATIONDEFERRAL_003"
    rule_name = "Geriatrics VaccinationDeferral Screening Rule #3"
    category = "GERIATRICS"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Geriatrics (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_geriatrics_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("geriatrics_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("geriatrics_vaccinationdeferral_lab_marker", 10.0)
