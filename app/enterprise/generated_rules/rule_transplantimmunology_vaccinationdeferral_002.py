"""
Enterprise Clinical Screening & Medical Rule Engine: TransplantImmunology - VaccinationDeferral (2).
...
class TransplantImmunologyVaccinationDeferralClinicalProcessor2:
    """
    Detailed decision processor evaluating TransplantImmunology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_TRANSPLANTIMMUNOLOGY_VACCINATIONDEFERRAL_002"
    rule_name = "TransplantImmunology VaccinationDeferral Screening Rule #2"
    category = "TRANSPLANTIMMUNOLOGY"
    sub_category = "VACCINATIONDEFERRAL"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for TransplantImmunology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_transplantimmunology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("transplantimmunology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("transplantimmunology_vaccinationdeferral_lab_marker", 10.0)
