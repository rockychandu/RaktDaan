"""
Enterprise Clinical Screening & Medical Rule Engine: Neurology - EndemicTravelExposure (1).
...
class NeurologyEndemicTravelExposureClinicalProcessor1:
    """
    Detailed decision processor evaluating Neurology (EndemicTravelExposure) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_NEUROLOGY_ENDEMICTRAVELEXPOSURE_001"
    rule_name = "Neurology EndemicTravelExposure Screening Rule #1"
    category = "NEUROLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Neurology (EndemicTravelExposure)."""
        has_primary = donor_payload.get("has_neurology_endemictravelexposure_primary", False)
        severity_score = donor_payload.get("neurology_endemictravelexposure_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("neurology_endemictravelexposure_lab_marker", 10.0)
