"""
Enterprise Clinical Screening & Medical Rule Engine: Gastroenterology - EndemicTravelExposure (2).
...
class GastroenterologyEndemicTravelExposureClinicalProcessor2:
    """
    Detailed decision processor evaluating Gastroenterology (EndemicTravelExposure) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_GASTROENTEROLOGY_ENDEMICTRAVELEXPOSURE_002"
    rule_name = "Gastroenterology EndemicTravelExposure Screening Rule #2"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Gastroenterology (EndemicTravelExposure)."""
        has_primary = donor_payload.get("has_gastroenterology_endemictravelexposure_primary", False)
        severity_score = donor_payload.get("gastroenterology_endemictravelexposure_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("gastroenterology_endemictravelexposure_lab_marker", 10.0)
