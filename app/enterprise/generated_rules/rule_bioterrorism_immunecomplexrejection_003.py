"""
Enterprise Clinical Screening & Medical Rule Engine: Bioterrorism - ImmuneComplexRejection (3).
...
class BioterrorismImmuneComplexRejectionClinicalProcessor3:
    """
    Detailed decision processor evaluating Bioterrorism (ImmuneComplexRejection) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_BIOTERRORISM_IMMUNECOMPLEXREJECTION_003"
    rule_name = "Bioterrorism ImmuneComplexRejection Screening Rule #3"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Bioterrorism (ImmuneComplexRejection)."""
        has_primary = donor_payload.get("has_bioterrorism_immunecomplexrejection_primary", False)
        severity_score = donor_payload.get("bioterrorism_immunecomplexrejection_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("bioterrorism_immunecomplexrejection_lab_marker", 10.0)
