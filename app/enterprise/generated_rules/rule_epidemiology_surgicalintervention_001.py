"""
Enterprise Clinical Screening & Medical Rule Engine: Epidemiology - SurgicalIntervention (1).
...
class EpidemiologySurgicalInterventionClinicalProcessor1:
    """
    Detailed decision processor evaluating Epidemiology (SurgicalIntervention) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_EPIDEMIOLOGY_SURGICALINTERVENTION_001"
    rule_name = "Epidemiology SurgicalIntervention Screening Rule #1"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Epidemiology (SurgicalIntervention)."""
        has_primary = donor_payload.get("has_epidemiology_surgicalintervention_primary", False)
        severity_score = donor_payload.get("epidemiology_surgicalintervention_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("epidemiology_surgicalintervention_lab_marker", 10.0)
