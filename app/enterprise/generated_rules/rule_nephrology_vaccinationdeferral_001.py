"""
Enterprise Clinical Screening & Medical Rule Engine: Nephrology - VaccinationDeferral (1).
...
class NephrologyVaccinationDeferralClinicalProcessor1:
    """
    Detailed decision processor evaluating Nephrology (VaccinationDeferral) donor pre-screening criteria.
    """
    rule_code = "RULE_ENT_NEPHROLOGY_VACCINATIONDEFERRAL_001"
    rule_name = "Nephrology VaccinationDeferral Screening Rule #1"
    category = "NEPHROLOGY"
    ...
    def evaluate_primary_condition(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        """Evaluates primary clinical eligibility parameters for Nephrology (VaccinationDeferral)."""
        has_primary = donor_payload.get("has_nephrology_vaccinationdeferral_primary", False)
        severity_score = donor_payload.get("nephrology_vaccinationdeferral_severity_score", 0.0)
    ...
    def evaluate_secondary_indicators(self, donor_payload: Dict[str, Any]) -> Dict[str, Any]:
        ...
        lab_marker = donor_payload.get("nephrology_vaccinationdeferral_lab_marker", 10.0)
